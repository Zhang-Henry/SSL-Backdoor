...
def add_watermark(net,input_image_path,val=False):
    ...
    img_backdoor = x_tensor.squeeze().detach().numpy()

    img_backdoor = np.clip(img_backdoor, 0, 1) #限制颜色范围在0-1
    scaled_image = (img_backdoor * 255).astype(np.uint8)
...
def generate_poison(class_list, source_path, poisoned_destination_path,
    ...
    unet = torch.load('/home/hrzhang/projects/SSL-Backdoor/optimize_filter/trigger/moco/unet_2023-09-21-00-10-57-ssim97.pt')
