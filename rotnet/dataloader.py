...
_IMAGENET100_DATASET_DIR = '/home/aniruddha/imagenet100/'
...
class FileListDataset(data.Dataset):
    def __init__(self, path_to_txt_file, transform=None):
        ...
        with open(path_to_txt_file, 'r') as f:
            ...
            # ada hack TODO: Aniruddha
            myhost = os.uname()[1]
            if 'g' in myhost:
                self.file_list = [row.replace('/home/aniruddha/', '/nfs/ada/hpirsiav/users/anisaha1/data/') for row in self.file_list]
...
class GenericDataset(data.Dataset):
    def __init__(self, dataset_name, split, file_list, random_sized_crop=False,
        ...
        # TODO: ANIRUDDHA ImageNet100
        elif self.dataset_name=='imagenet100':
            assert(self.split=='train' or self.split=='val' or self.split=='val_poisoned')
            self.mean_pix = [0.485, 0.456, 0.406]
            self.std_pix = [0.229, 0.224, 0.225]

            if self.split!='train':
                if self.split=='val_poisoned':
                    transforms_list = [
                    lambda x: np.asarray(x),
                ]
                else:
            ...
            self.transform = transforms.Compose(transforms_list)
            split_data_dir = _IMAGENET100_DATASET_DIR + '/' + self.split
            # self.data = datasets.ImageFolder(split_data_dir, self.transform)
            # TODO: Aniruddha
            self.data = FileListDataset(file_list, self.transform)
