...
def eval_sgd(x_train, y_train, x_test, y_test, x_test_p=None, y_test_p=None, clf_fname=None, chkpt=None, evaluate=False, topk=[1, 5], epoch=500):
...
def eval_sgd2(x_train, y_train, x_test, y_test, topk=[1, 5], epoch=500):
    """ linear classifier accuracy (sgd) """
    lr_start, lr_end = 1e-2, 1e-6
    gamma = (lr_end / lr_start) ** (1 / epoch)
    output_size = x_train.shape[1]
