...
class Solver():
    ...
    def train_one_epoch(self,args,recorder,bar,tracker,train_loader):
        ...
        for img,img_trans,_ in train_loader:
        ...
        if ssim >= args.ssim_threshold and psnr >= args.psnr_threshold and lp <= args.lp_threshold and color >= recorder.best:
            ...
            torch.save(state, f'trigger/moco/{self.args.timestamp}/ssim{ssim:.4f}_psnr{psnr:.2f}_lp{lp:.4f}_wd{wd:.3f}_color{color:.3f}.pt')
            recorder.best = color
            print('\n--------------------------------------------------')
